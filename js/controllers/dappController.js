...
angular.module('webApp').controller('dappController', ['$scope', 'viewFactory', '$stateParams', '$http', "$window", "$interval", "userService", "peerFactory",
    function ($scope, viewFactory, $stateParams, $http, $window, $interval, userService, peerFactory) {
        ...
        $scope.isInstalled = function () {
            $http.get(peerFactory.getUrl() + '/api/dapps/installedIds').then(function (response) {
        ...
        $scope.getInstalling = function () {
            $http.get(peerFactory.getUrl() + "api/dapps/installing").then(function (response) {
        ...
        $scope.getRemoving = function () {
            $http.get(peerFactory.getUrl() + "/api/dapps/removing").then(function (response) {
                if (response.data.success) {
        ...
        $scope.getLaunched = function () {
            $http.get(peerFactory.getUrl() + "/api/dapps/launched").then(function (response) {
                if (response.data.success) {
        ...
        $scope.runDApp = function () {
            // open dapp
            $http.post(peerFactory.getUrl() +"/api/dapps/launch", {
