...
angular.module('webApp').controller('dappsCategoryController', ['$scope', 'viewFactory', '$http', '$stateParams', 'dappsService', '$timeout', 'peerFactory',
    function ($scope, viewFactory, $http, $stateParams, dappsService, $timeout, peerFactory) {
        ...
        $scope.searchDappText = function () {
            if ($scope.category == 'Installed') {
                if ($scope.searchDapp.searchForDapp.trim() != '') {
                    $http.get(peerFactory.getUrl() + "/api/dapps/search?q=" + $scope.searchDapp.searchForDapp + "&installed=1").then(function (response) {
                        console.log("here");
                ...
                if ($scope.searchDapp.searchForDapp.trim() != '') {
                    $http.get(peerFactory.getUrl() + "/api/dapps/search?q=" + $scope.searchDapp.searchForDapp + "&category=" + $scope.category).then(function (response) {
                        $scope.dapps = response.data.dapps;
                        $scope.searchDapp.inSearch = false;
                        $scope.view.inLoading = false;
                        $scope.searchedText = '(search for "' + $scope.searchDapp.searchForDapp + '")';
                    ...
                    $http.get(peerFactory.getUrl() + "/api/dapps/?type=1&category=" + $scope.category).then(function (response) {
                        $scope.dapps = response.data.dapps;
                        $scope.searchDapp.inSearch = false;
                        $scope.searchedText = '';
                        $scope.view.inLoading = false;
